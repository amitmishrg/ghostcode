...
export function DialogSearchList<T>({
  ...
  const visibleHeight = Math.min(filtered.length, MAX_VISIBLE_ITEMS) * itemHeight;
  ...
  useKeyboard((key) => {
    ...
    if (key.name === 'return' || key.name === 'enter') {
      ...
      setSelectedIndex((i) => {
        ...
        if (sb && newIndex * itemHeight < sb.scrollTop) {
          sb.scrollTo(newIndex * itemHeight);
        }
      ...
      setSelectedIndex((i) => {
        ...
        if (sb) {
          ...
          const visibleEnd = sb.scrollTop + viewportHeight - itemHeight;
          if (itemTop > visibleEnd) {
            sb.scrollTo(itemTop - viewportHeight + itemHeight);
          }
        }
        const item = filtered[newIndex];
        if (item && onHighlight) onHighlight(item);
